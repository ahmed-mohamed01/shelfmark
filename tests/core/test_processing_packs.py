...
def test_multifile_book_inside_pack_keeps_original_filenames_per_book(tmp_path):
    # Branch behaviour (935f7b9): a multipart audiobook keeps its original filenames
    # inside the template folder instead of upstream's `Title - NN` part numbering.
    # What matters for packs is that each book's files stay in that book's own tree.
    staging = tmp_path / "staging"
    ingest = tmp_path / "ingest"
    staging.mkdir()
    ingest.mkdir()
    ...
    assert sorted(p.name for p in one.rglob("*.mp3")) == ["part1.mp3", "part2.mp3", "part3.mp3"]
...
def test_without_pack_fields_nested_pack_is_still_one_book(tmp_path):
    ...
    assert result is not None
    author_dir = ingest / "James S. A. Corey"
    # One book under the searched title (branch keeps original filenames, see above),
    # and no per-book folders for the pack's own titles.
    drive = author_dir / "Drive"
    assert sorted(p.name for p in drive.rglob("*.m4b")) == ["empire.m4b", "howling.m4b"]
    assert sorted(p.name for p in author_dir.iterdir()) == ["Drive"]
