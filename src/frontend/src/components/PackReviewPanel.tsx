...
interface PackReviewPanelProps {
  ...
  /** Confirm-button verb: "Download" queues right away; the selection modal passes "Use". */
  actionVerb?: string;
}
...
export const PackReviewPanel = ({
  ...
  const confirmLabel = singleBook
    ? `${actionVerb} as one book`
    : `${actionVerb} ${payloadBooks.length} ${payloadBooks.length === 1 ? 'book' : 'books'}`;
