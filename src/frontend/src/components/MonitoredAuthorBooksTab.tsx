...
const extractBookPopularity = (
  ...
  for (const field of fields) {
    ...
    if (
      readersCount === null &&
      (icon === 'users' || /readers?|users?|followers?|people/.test(label))
    ) {
...
const withMonitoredAvailability = (book: Book, rows: MonitoredBookRow[]): Book => {
  ...
  return {
    ...
    has_ebook_candidate: isEnabledMonitoredFlag(row.has_ebook_candidate),
    has_audiobook_candidate: isEnabledMonitoredFlag(row.has_audiobook_candidate),
...
export const MonitoredAuthorBooksTab = ({
  ...
  const [booksCompactMinWidth, setBooksCompactMinWidth] = useState<number>(() => {
    ...
    return Math.max(
      AUTHOR_BOOKS_COMPACT_MIN_WIDTH_MIN,
      Math.min(AUTHOR_BOOKS_COMPACT_MIN_WIDTH_MAX, parsed),
    );
  });
  ...
  const [booksFilters, setBooksFilters] = useState<AuthorBooksFilters>(() =>
    createDefaultAuthorBooksFilters(),
  );
  ...
  const [bulkDownloadRunningByType, setBulkDownloadRunningByType] = useState<
  ...
  useEffect(() => {
    ...
    const match = books.find(
      (book) => (book.provider || '') === provider && (book.provider_id || '') === providerId,
    );
    if (match) setActiveBookDetails(withMonitoredAvailability(match, monitoredBookRows));
    setHasAppliedInitialBookSelection(true);
  }, [
    books,
    hasAppliedInitialBookSelection,
    initialBookProvider,
    initialBookProviderId,
    isLoadingBooks,
    monitoredBookRows,
  ...
  useEffect(() => {
    ...
    const completionSignature = relevantCompleted
      ...
      .sort()
      .join('|');
    if (!completionSignature || completionSignature === lastAutoRefreshSignatureRef.current.value)
      return;
  ...
  useEffect(() => {
    setActiveBookDetails((prev) =>
      prev ? withMonitoredAvailability(prev, monitoredBookRows) : prev,
    );
  }, [monitoredBookRows]);
  ...
  useEffect(() => {
    try {
      localStorage.setItem('authorBooksGroup', booksGroup);
    } catch {
      /* ignore */
    }
  }, [booksGroup]);
  ...
  useEffect(() => {
    try {
      localStorage.setItem('authorBooksSortAsc', String(booksSortAsc));
    } catch {
      /* ignore */
    }
  }, [booksSortAsc]);
      ...
      additional_series: (() => {
        ...
        const raw =
          ...
          (() => {
            ...
            try {
              const p = JSON.parse(row.all_series);
              return Array.isArray(p)
                ? (p as Array<{ name: string; position?: number; count?: number }>)
                : undefined;
            } catch {
              return undefined;
            }
  ...
  useEffect(() => {
    setSelectedBookIds((prev) => {
      ...
      for (const book of books) {
        if (prev[book.id]) next[book.id] = true;
      }
  ...
  const groupedBooks = useMemo(() => {
    ...
    const withinGroupSort = (a: Book, b: Book) => {
      ...
      if (booksSort === 'popularity' || booksSort === 'rating') {
        ...
        if (booksSort === 'popularity') {
          const aR = aP.readersCount ?? -1;
          const bR = bP.readersCount ?? -1;
          if (bR !== aR) return dir * (bR - aR);
          const aRc = aP.ratingsCount ?? -1;
          const bRc = bP.ratingsCount ?? -1;
          if (bRc !== aRc) return dir * (bRc - aRc);
          const aRt = aP.rating ?? -1;
          const bRt = bP.rating ?? -1;
    ...
    if (booksGroup === 'year') {
      ...
      const years = Array.from(yearMap.keys()).sort((a, b) =>
        yearAsc ? Number(a) - Number(b) : Number(b) - Number(a),
      );
    ...
    for (const book of visibleBooks) {
      ...
      else seriesMap.set(primarySeriesKey, [book]);
      if (showBooksInMultipleSeries && book.additional_series) {
        for (const alt of book.additional_series) {
          ...
          if (altList) altList.push(book);
          else seriesMap.set(altKey, [book]);
        }
      }
    }
  ...
  const getMonitoredAvailabilityForBook = useCallback(
    (
      book: Book,
    ): {
      hasEbook: boolean;
      hasAudiobook: boolean;
      hasEbookCandidate: boolean;
      hasAudiobookCandidate: boolean;
      ebookFormat?: string;
      ...
      // Candidate flags come from the monitored row only — bare Book objects
      // from search results don't carry them.
      const hasEbookCandidate = isEnabledMonitoredFlag(row?.has_ebook_candidate);
      const hasAudiobookCandidate = isEnabledMonitoredFlag(row?.has_audiobook_candidate);
      return {
        hasEbook: monitoredBookHasFormatAvailable(source, 'ebook'),
        hasAudiobook: monitoredBookHasFormatAvailable(source, 'audiobook'),
        hasEbookCandidate,
        hasAudiobookCandidate,
        ebookFormat:
          typeof ebookFormatRaw === 'string' && ebookFormatRaw.trim()
            ? ebookFormatRaw.trim().toLowerCase()
            : undefined,
        audiobookFormat:
          typeof audiobookFormatRaw === 'string' && audiobookFormatRaw.trim()
  ...
  const filteredGroupedBooks = useMemo(() => {
    ...
    const groups = groupedBooks
      ...
      .map((group) => {
        ...
        const isDormantGroup =
          nextBooks.length > 0 && nextBooks.every((book) => isDormantBookInGroup(book));
        const isRemovedGroup =
          nextBooks.length > 0 &&
          nextBooks.every((book) => {
  ...
  const singleActiveFilterLabel = useMemo(() => {
    ...
    if (booksFilters.showUpcoming)
      labels.push(
        booksFilters.upcomingWindow === 'any'
          ? 'Upcoming'
          : UPCOMING_WINDOW_LABELS[booksFilters.upcomingWindow],
      );
    ...
    if (booksFilters.seriesKeys.length > 0) {
      if (booksFilters.seriesKeys.length === 1) {
        const selected = seriesFilterOptions.find(
          (option) => option.key === booksFilters.seriesKeys[0],
        );
  ...
  const activeFilterChips = useMemo(() => {
    ...
    if (booksFilters.availability !== 'all')
      chips.push({
        key: 'availability',
        label: AVAILABILITY_FILTER_LABELS[booksFilters.availability],
      });
    if (booksFilters.showUpcoming)
      chips.push({
        key: 'upcoming',
  ...
  const visibleBooks = useMemo(
    () => filteredGroupedBooks.flatMap((group) => group.books),
    [filteredGroupedBooks],
  );
  const allVisibleBooksSelected = useMemo(
    () =>
      visibleBooks.length > 0 && visibleBooks.every((book) => Boolean(selectedBookIds[book.id])),
    [visibleBooks, selectedBookIds],
  );
  const selectedBooks = useMemo(
    () => books.filter((book) => Boolean(selectedBookIds[book.id])),
  ...
  const toggleSelectAllVisibleBooks = useCallback(() => {
    ...
    setSelectedBookIds((prev) => {
      ...
      const next = { ...prev };
      if (allSelected) {
        for (const book of visibleBooks) delete next[book.id];
      } else {
        for (const book of visibleBooks) next[book.id] = true;
      }
      return next;
    });
  }, [visibleBooks]);

  const runBulkDownloadForBooks = useCallback(
    async (booksToDownload: Book[], contentType: ContentType) => {
      if (!onGetReleases || booksToDownload.length === 0 || bulkDownloadRunningByType[contentType])
        return;
  ...
  const runBulkDownloadForSelection = useCallback(
  ...
  const runBulkInteractiveSearchForSelection = useCallback(
    async (contentType: ContentType) => {
  ...
  const handleRunMonitoredSearch = useCallback(
    async (contentType: ContentType) => {
  ...
  const toggleSelectAllInGroup = useCallback((groupBooks: Book[]) => {
    ...
    setSelectedBookIds((prev) => {
      ...
      const next = { ...prev };
      if (allSelected) {
        for (const book of groupBooks) delete next[book.id];
      } else {
        for (const book of groupBooks) next[book.id] = true;
      }
      return next;
    });
  }, []);
  ...
  const isBookDormant = useCallback(
    (book: Book): boolean => {
  ...
  const toggleBookMonitor = useCallback(
    async (book: Book, type: 'ebook' | 'audiobook' | 'both', newValue?: boolean) => {
      ...
      } = { provider, provider_book_id: providerId };
      if (type === 'ebook') {
        ...
        const targetValue =
          newValue !== undefined ? newValue : !(current.monitorEbook && current.monitorAudiobook);
        ...
        setMonitoredBookRows((prev) =>
          prev.map((r) =>
            r.provider === provider && r.provider_book_id === providerId
              ? {
                  ...r,
                  monitor_ebook: current.monitorEbook,
                  monitor_audiobook: current.monitorAudiobook,
                }
              : r,
          ),
        );
  ...
  const toggleBookHidden = useCallback(
    async (book: Book) => {
      ...
      const snapshot: Pick<
        ...
        | 'monitor_audiobook'
        | 'saved_monitor_ebook'
        | 'saved_monitor_audiobook'
      > | null = currentRow
        ? {
            hidden: currentRow.hidden,
            monitor_ebook: currentRow.monitor_ebook,
            monitor_audiobook: currentRow.monitor_audiobook,
            saved_monitor_ebook: currentRow.saved_monitor_ebook,
            saved_monitor_audiobook: currentRow.saved_monitor_audiobook,
          }
        : null;
      setMonitoredBookRows((prev) =>
        prev.map((r) => {
          ...
          return {
            ...r,
            hidden: false,
            monitor_ebook: restoredEbook,
            monitor_audiobook: restoredAudio,
      ...
      try {
        const resp = await updateMonitoredBooksMonitorFlags(monitoredEntityId, {
          provider,
          ...
          prev.map((r) =>
            r.provider === provider && r.provider_book_id === providerId && snapshot
              ? {
                  ...
                  monitor_ebook: snapshot.monitor_ebook,
                  monitor_audiobook: snapshot.monitor_audiobook,
                  saved_monitor_ebook: snapshot.saved_monitor_ebook,
                  saved_monitor_audiobook: snapshot.saved_monitor_audiobook,
                }
              : r,
          ),
        );
  ...
  const renderBookActionMenuContent = useCallback(
    (book: Book, defaultContentType: ContentType, defaultAction: ReleasePrimaryAction) =>
      ({ close }: { close: () => void }) => {
        const monitorState = getBookMonitorState(book);
            ...
            {onGetReleases ? (
              <>
                ...
                {SEARCH_DROPDOWN_OPTIONS.map((option) => {
                  const isDefault =
                    option.contentType === defaultContentType && option.action === defaultAction;
                  return (
                    <button
                      type="button"
                      ...
                      <button
                        ...
                        onClick={async () => {
                          ...
                          void deleteMonitoredBook(monitoredEntityId!, p, pid).then(() => {
                            setMonitoredBookRows((prev) =>
                              prev.filter((b) => !(b.provider === p && b.provider_book_id === pid)),
                            );
                          });
                        }}
  ...
  const renderBookOverflowMenu = useCallback(
    (book: Book) => {
  ...
  const renderBookTableActions = useCallback(
    (book: Book) => {
      const { defaultContentType, defaultAction, isAutoDefault, primaryLabel } =
        getDefaultBookSearchMode();
      return (
        <div className="inline-flex items-stretch rounded-lg border border-emerald-500/40">
          <button
            ...
            className="hover-action inline-flex h-8 w-8 items-center justify-center text-emerald-600 dark:text-emerald-400"
          ...
          >
            <span className="relative inline-flex items-center justify-center">
              {releaseCombinedMode ? (
                ...
                >
                  ...
                  <path
                    strokeLinecap="round"
                    strokeLinejoin="round"
                    d="M10 8.5A7 7 0 0 0 5.3 7C4.5 7 3.7 7.13 3 7.38v11.5c.7-.25 1.5-.38 2.3-.38 1.8 0 3.4.67 4.7 1.8m0-11.8A7 7 0 0 1 14.7 7c.8 0 1.5.1 2.1.3"
                  />
              ...
              {isAutoDefault ? (
                <svg
                  className="absolute -right-1.5 -bottom-1 h-2.5 w-2.5"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                  strokeWidth={3}
                ...
                >
                  <path
                    strokeLinecap="round"
                    strokeLinejoin="round"
                    d="m21 21-4.35-4.35m1.35-5.15a5 5 0 1 1-10 0 5 5 0 0 1 10 0Z"
                  />
                </svg>
              )}
            </span>
          ...
          <Dropdown
            ...
            renderTrigger={({ isOpen, toggle }) => (
              <button
                type="button"
                onClick={toggle}
                className={`hover-action inline-flex h-8 w-7 items-center justify-center border-l border-emerald-500/30 text-emerald-600 dark:text-emerald-400 ${isOpen ? 'bg-emerald-500/10' : ''}`}
                aria-label={`More actions for ${book.title || 'this book'}`}
                title="More actions"
              >
                <svg
  ...
  return (
    <>
      <div className="mt-4 border-t border-gray-200/60 pt-4 dark:border-gray-700/60">
        <div
          ref={booksToolbarRef}
          className={`${compactToolbar ? '' : 'sticky'} ${compactToolbar ? 'top-0 z-10' : isPageMode ? 'top-[76px] z-[45]' : 'top-0 z-40'} ${isBooksToolbarPinned && !compactToolbar ? '-mr-[100vw] -ml-[100vw] border-0 border-b border-[var(--border-muted)] bg-[var(--bg)] px-[100vw]' : ''}`}
        >
          <div className="flex flex-wrap items-center gap-2 gap-y-1.5 px-3 py-2">
            <div className="flex min-w-0 items-center gap-2">
              <button
                ...
                className="hover-action rounded-full p-1.5 text-gray-500 transition-all duration-200 hover:text-gray-900 dark:hover:text-gray-100"
                aria-label={
                  allGroupsCollapsed ? 'Expand all series groups' : 'Collapse all series groups'
                }
              ...
              <div className="relative min-w-0 overflow-hidden">
                {/* Static label: "BOOKS" */}
                <p
                  className={`truncate text-[11px] tracking-wide uppercase transition-all duration-300 ${isBooksToolbarPinned ? 'pointer-events-none absolute -translate-y-3 opacity-0 select-none' : 'text-gray-500 dark:text-gray-400'}`}
                >
                  Books
                  {isRefreshing ? (
                ...
                <p
                  className={`truncate text-sm font-semibold transition-all duration-300 ${isBooksToolbarPinned ? 'translate-y-0 text-gray-900 opacity-100 dark:text-gray-100' : 'pointer-events-none absolute translate-y-3 opacity-0 select-none'}`}
                >
            ...
            <div className="ml-auto flex flex-shrink-0 items-center gap-2">
              {!isPageMode ? (
                <div
                  className="hidden items-center gap-2 rounded-full border border-[var(--border-muted)] px-2.5 py-1.5 sm:flex"
                ...
                >
                  ...
                  <input
                    ...
                    className="w-44 bg-transparent text-xs text-gray-700 outline-none placeholder:text-gray-500 dark:text-gray-200"
                  ...
                  {activeBooksQuery ? (
                    <button
                      ...
                      className="hover-action rounded-full p-0.5 text-gray-500 hover:text-gray-900 dark:hover:text-gray-100"
                ...
                renderTrigger={({ isOpen, toggle }) => (
                  <div className="inline-flex items-center gap-1.5">
                    <button
                      type="button"
                      onClick={toggle}
                      className={`relative rounded-full p-1.5 transition-all duration-200 ${isOpen || activeFiltersCount > 0 ? 'text-emerald-600 dark:text-emerald-400' : 'hover-action text-gray-500 hover:text-gray-900 dark:hover:text-gray-100'}`}
                      aria-haspopup="menu"
                      aria-expanded={isOpen}
                      aria-label={
                        singleActiveFilterLabel
                          ? `Filter books (${singleActiveFilterLabel})`
                          : 'Filter books'
                      }
                      title={
                        singleActiveFilterLabel
                    ...
                    >
                      ...
                      >
                        <path
                          strokeLinecap="round"
                          strokeLinejoin="round"
                          d="M3 4.5h18l-7 8.25v5.25l-4 2.25v-7.5L3 4.5Z"
                        />
                      </svg>
                      {activeFiltersCount > 1 ? (
                        <span className="pointer-events-none absolute -top-1 -right-1 inline-flex h-4 min-w-4 items-center justify-center rounded-full bg-emerald-600 px-1 text-[10px] font-semibold text-white">
                          {activeFiltersCount}
                        </span>
                      ) : null}
                    </button>
                    {singleActiveFilterLabel ? (
                      <span className="max-w-28 truncate rounded-full border border-[var(--border-muted)] bg-emerald-50/70 px-2 py-0.5 text-[10px] font-medium text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-300">
                        {singleActiveFilterLabel}
                      </span>
                    ) : null}
                  </div>
              ...
              >
                {({ close }) => (
                  <div className="py-1" role="menu" aria-label="Book filters">
                    <div className="px-3 pb-1 text-[10px] tracking-wide text-gray-500 uppercase dark:text-gray-400">
                      Availability
                    </div>
                    <div className="relative">
                      <button
                        type="button"
                        className={`hover-surface flex w-full items-center justify-between px-3 py-2 text-left text-sm ${AVAILABLE_FILTER_MODES.includes(booksFilters.availability) ? 'font-medium text-emerald-600 dark:text-emerald-400' : ''}`}
                        onClick={() => {
                      ...
                      >
                        <span>Available</span>
                        <svg
                          className={`h-3.5 w-3.5 transition-transform ${isAvailabilityFilterMenuOpen ? 'rotate-90' : ''}`}
                          fill="none"
                          stroke="currentColor"
                          viewBox="0 0 24 24"
                          strokeWidth={1.8}
                      ...
                      {isAvailabilityFilterMenuOpen ? (
                        <div className="pr-2 pb-1 pl-3">
                          {AVAILABLE_FILTER_MODES.map((mode) => (
                            <button
                              key={mode}
                              type="button"
                              className={`hover-surface flex w-full items-center justify-between rounded-md px-3 py-1.5 text-left text-xs ${booksFilters.availability === mode ? 'font-medium text-emerald-600 dark:text-emerald-400' : 'text-gray-600 dark:text-gray-300'}`}
                              onClick={() => {
                    ...
                    <div className="relative">
                      <button
                        type="button"
                        className={`hover-surface flex w-full items-center justify-between px-3 py-2 text-left text-sm ${MISSING_FILTER_MODES.includes(booksFilters.availability) ? 'font-medium text-emerald-600 dark:text-emerald-400' : ''}`}
                        onClick={() => {
                      ...
                      >
                        <span>Missing</span>
                        <svg
                          className={`h-3.5 w-3.5 transition-transform ${isMissingFilterMenuOpen ? 'rotate-90' : ''}`}
                          fill="none"
                          stroke="currentColor"
                          viewBox="0 0 24 24"
                          strokeWidth={1.8}
                    ...
                    <button
                      type="button"
                      className={`hover-surface flex w-full items-center justify-between px-3 py-2 text-left text-sm ${booksFilters.showNoReleaseDate ? 'font-medium text-emerald-600 dark:text-emerald-400' : ''}`}
                      onClick={() => {
                        setBooksFilters((prev) => ({
                          ...prev,
                          showNoReleaseDate: !prev.showNoReleaseDate,
                        }));
                      }}
                    >
                    ...
                    <button
                      type="button"
                      className={`hover-surface flex w-full items-center justify-between px-3 py-2 text-left text-sm ${booksFilters.showUpcoming ? 'font-medium text-emerald-600 dark:text-emerald-400' : ''}`}
                      onClick={() => {
                    ...
                    {booksFilters.showUpcoming ? (
                      <div className="pr-2 pb-1 pl-3">
                        {(['any', '30d', '90d', 'this_year'] as UpcomingWindowMode[]).map(
                          (windowMode) => (
                            <button
                              key={windowMode}
                              type="button"
                              className={`hover-surface flex w-full items-center justify-between rounded-md px-3 py-1.5 text-left text-xs ${booksFilters.upcomingWindow === windowMode ? 'font-medium text-emerald-600 dark:text-emerald-400' : 'text-gray-600 dark:text-gray-300'}`}
                              onClick={() => {
                                setBooksFilters((prev) => ({
                                  ...prev,
                                  upcomingWindow: windowMode,
                                }));
                              }}
                            >
                    ...
                    <div className="relative">
                        ...
                        <span className="inline-flex items-center gap-2">
                          ...
                          {booksFilters.seriesKeys.length > 0 ? (
                            <span className="rounded-full bg-emerald-600 px-1.5 py-0.5 text-[10px] text-white">
                              {booksFilters.seriesKeys.length}
                            </span>
                          ) : null}
                        </span>
                        <svg
                          className={`h-3.5 w-3.5 transition-transform ${isSeriesFilterMenuOpen ? 'rotate-90' : ''}`}
                          fill="none"
                          stroke="currentColor"
                          viewBox="0 0 24 24"
                          strokeWidth={1.8}
                      ...
                      {isSeriesFilterMenuOpen ? (
                        <div className="absolute top-0 left-full z-10 ml-2 max-h-56 w-56 overflow-auto rounded-lg border border-[var(--border-muted)] bg-[var(--bg)] py-1 shadow-xl">
                            ...
                            seriesFilterOptions.map((option) => {
                              ...
                              return (
                                <button
                                  key={option.key}
                                  type="button"
                                  className={`hover-surface flex w-full items-center justify-between px-3 py-2 text-left text-sm ${selected ? 'font-medium text-emerald-600 dark:text-emerald-400' : ''}`}
                                  onClick={() => {
                                    toggleSeriesFilter(option.key);
                                    if (closeSeriesFilterOnSelect) setIsSeriesFilterMenuOpen(false);
                                  }}
                                >
                          ...
                          <div className="mt-1 border-t border-[var(--border-muted)] pt-1">
                            <button
                              type="button"
                              className={`hover-surface flex w-full items-center justify-between px-3 py-2 text-left text-xs ${closeSeriesFilterOnSelect ? 'text-emerald-600 dark:text-emerald-400' : 'text-gray-500 dark:text-gray-400'}`}
                    ...
                    <div className="flex items-center justify-between gap-2 px-2 pt-2 pb-1">
                      <button
                        type="button"
                        className="hover-surface rounded-md px-2.5 py-1 text-xs"
                        onClick={() => {
              ...
              {!compactToolbar && (
                <Dropdown
                  ...
                  renderTrigger={({ isOpen, toggle }) => (
                    <button
                      type="button"
                      onClick={toggle}
                      className={`rounded-full p-1.5 transition-all duration-200 ${isOpen ? 'text-gray-900 dark:text-gray-100' : 'hover-action text-gray-500 hover:text-gray-900 dark:hover:text-gray-100'}`}
                      aria-haspopup="listbox"
                      aria-expanded={isOpen}
                      title="Sort & group books"
                    >
                      <svg
                        className="h-4 w-4"
                        fill="none"
                ...
                >
                  {() => (
                    <div>
                      <div className="px-3 pt-2 pb-1 text-[10px] font-semibold tracking-wider text-gray-400 uppercase dark:text-gray-500">
                        Group
                      </div>
                      {[
                        ...
                        return (
                          <button
                            key={`group-${key}`}
                            type="button"
                            className={`hover-surface flex w-full items-center justify-between px-3 py-1.5 text-left text-sm ${active ? 'font-medium text-emerald-600 dark:text-emerald-400' : ''}`}
                            onClick={() => setBooksGroup(key)}
                          >
                            <span>{label}</span>
                            {active && (
                              <svg
                      ...
                      <div className="px-3 pt-1 pb-1 text-[10px] font-semibold tracking-wider text-gray-400 uppercase dark:text-gray-500">
                        Sort
                      </div>
                      {[
                        ...
                        return (
                          <button
                            ...
                            onClick={() => {
                              if (active) {
                                setBooksSortAsc((prev) => !prev);
                              } else {
                                setBooksSort(key);
                                setBooksSortAsc(key !== 'popularity' && key !== 'rating');
                              }
                            }}
                            role="option"
                          ...
                          >
                            ...
                            {active && (
                              <svg
                                className={`h-3.5 w-3.5 transition-transform ${booksSortAsc ? '' : 'rotate-180'}`}
                                fill="none"
                                stroke="currentColor"
                                viewBox="0 0 24 24"
                                strokeWidth={2}
                  ...
                  >
                    ...
                    >
                      <path
                        strokeLinecap="round"
                        strokeLinejoin="round"
                        d="M4 19.5h16M7.5 4.5v12m4.5-9v9m4.5-6v6"
                      />
                    </svg>
                  </button>
                )}
                  ...
                  <div className="py-1">
                    <div className="px-3 py-2">
                      ...
                      <div className="mt-1 text-right text-[11px] text-gray-500 tabular-nums dark:text-gray-400">
                        {booksCompactMinWidth}px
                      </div>
                      {booksViewMode !== 'compact' ? (
                        <div className="mt-2 text-[11px] text-gray-500 dark:text-gray-400">
                          Switch to compact view to adjust tile size.
                        </div>
                      ) : null}
                    </div>
                  </div>
              ...
              {!compactToolbar && (
                <button
                  ...
                  className="hover-action flex-shrink-0 rounded-full p-1.5 text-gray-500 transition-all duration-200 hover:text-gray-900 disabled:cursor-not-allowed disabled:opacity-30 dark:hover:text-gray-100"
                  aria-label={
                    monitoredEntityId ? 'Refresh & scan files' : 'Refresh books from provider'
                  }
              ...
              {!compactToolbar && monitoredEntityId ? (
                <>
                  <button
                    ...
                    disabled={
                      bulkDownloadRunningByType.ebook || bulkDownloadRunningByType.audiobook
                    }
                    className="hover-action rounded-full border border-emerald-500/40 px-2.5 py-1 text-[11px] font-medium whitespace-nowrap text-emerald-600 disabled:opacity-40 dark:text-emerald-400"
                    title="Search monitored ebook candidates"
                  >
                    {bulkDownloadRunningByType.ebook ? (
                      'Searching…'
                    ) : (
                      <>
                        <span className="hidden sm:inline">Search </span>eBooks
                      </>
                    )}
                  </button>
                  <button
                  ...
                  >
                    {bulkDownloadRunningByType.audiobook ? (
                      'Searching…'
                    ) : (
                      <>
                        <span className="hidden sm:inline">Search </span>audiobooks
                      </>
                    )}
                  </button>
                </>
        ...
        <div
          className={`rounded-2xl bg-black/[0.03] dark:bg-white/[0.04] ${booksViewMode === 'compact' ? 'overflow-visible' : 'overflow-hidden'}`}
        >
            ...
            <div className="flex flex-wrap items-center gap-2 px-4 pt-3 pb-1">
              {activeFilterChips.map((chip) => (
                <button
                  key={chip.key}
                  type="button"
                  className="hover-surface inline-flex items-center gap-1.5 rounded-full border border-[var(--border-muted)] bg-[var(--bg-soft)] px-2.5 py-1 text-[11px]"
                  onClick={() => {
                    ...
                    if (chip.key.startsWith('series:')) {
                      ...
                      setBooksFilters((prev) => ({
                        ...prev,
                        seriesKeys: prev.seriesKeys.filter((key) => key !== seriesKey),
                      }));
                    }
                  }}
          ...
          <div className="py-2">
            ...
            {syncStatus === 'syncing' ? (
              <div className="mb-2 flex items-center gap-2 px-4 text-sm text-gray-500 dark:text-gray-400">
                <svg
                ...
                <span>
                  {syncPhase === 'fetching_books'
                    ? 'Fetching books…'
                    : syncPhase === 'scanning_files'
                      ? 'Scanning filesystem…'
                      : syncPhase === 'fetching_covers'
                        ? 'Fetching covers…'
                        : 'Syncing…'}
                </span>
              </div>
            ) : syncStatus === 'error' ? (
              <div className="mb-2 px-4 text-sm text-red-500">
                Sync error — try refreshing manually.
              </div>
            ) : null}
            {books.length === 0 && isLoadingBooks ? null : books.length === 0 &&
              !isLoadingBooks &&
              syncStatus !== 'syncing' ? (
              ...
              <>
                <div
                  key={booksViewMode}
                  className={`w-full ${booksViewMode === 'compact' ? 'flex flex-col gap-5 overflow-visible' : 'overflow-hidden'}`}
                >
                  {filteredGroupedBooks.map((group) => {
                    const isCollapsed = collapsedGroups[group.key] ?? false;
                    const isDormantGroup = Boolean((group as any).isDormantGroup);
                    const allSelectedInGroup = monitoredEntityId
                      ? group.books.length > 0 &&
                        group.books.every((book) => Boolean(selectedBookIds[book.id]))
                      : false;
                    let booksInSeries = 0;
                    let booksOnDisk = 0;
                    ...
                    if (monitoredEntityId) {
                      for (const book of group.books) {
                        ...
                        const pid = (book.provider_id || '').trim();
                        const row = p && pid ? monitoredBookRowByKey.get(`${p}:${pid}`) : undefined;
                        const bookUpcoming = row
                          ? isMonitoredBookUpcoming(row, _upcomingTodayMs)
                          : false;
                    ...
                    return (
                      <div key={group.key} data-series-key={group.key}>
                        <div
                          className={`flex items-center gap-2 border-b border-gray-200/60 px-3 pb-2 dark:border-gray-700/60 ${isDormantGroup ? 'opacity-60' : ''}`}
                        >
                          {monitoredEntityId ? (
                            <button
                              type="button"
                              onClick={() => {
                                hapticTap();
                                toggleSelectAllInGroup(group.books);
                              }}
                              className="flex-shrink-0 text-gray-500 transition-colors hover:text-gray-900 dark:hover:text-gray-100"
                              aria-label={
                                allSelectedInGroup
                                  ? `Unselect all books in ${group.title}`
                                  : `Select all books in ${group.title}`
                              }
                              title={
                                allSelectedInGroup
                                  ? 'Unselect all in series'
                          ...
                          <button
                            ...
                            className="hover-action flex w-fit min-w-0 items-center gap-2"
                            aria-expanded={!isCollapsed}
                          >
                            <svg
                              className={`h-3.5 w-3.5 flex-shrink-0 text-gray-400 transition-transform duration-200 dark:text-gray-500 ${isCollapsed ? '' : 'rotate-90'}`}
                              fill="none"
                              stroke="currentColor"
                              viewBox="0 0 24 24"
                              strokeWidth={2}
                            ...
                            {monitoredEntityId ? (
                              <span className="inline-flex items-center gap-1.5 text-[10px] font-medium text-gray-600 dark:text-gray-300">
                                <span
                                  className={`rounded-full bg-black/5 px-2 py-0.5 dark:bg-white/10 ${booksOnDisk > 0 ? 'text-emerald-600 dark:text-emerald-400' : ''}`}
                                >
                                  {booksOnDisk}/{booksInSeries}
                                </span>
                                {upcomingInGroup > 0 ? (
                                  <span className="text-amber-500 dark:text-amber-400">
                                    +{upcomingInGroup} upcoming
                                  </span>
                                ) : null}
                                {hiddenInGroup > 0 ? (
                              ...
                              <span className="inline-flex items-center rounded-full bg-black/5 px-2 py-0.5 text-[10px] font-medium text-gray-600 dark:bg-white/10 dark:text-gray-300">
                                {group.books.length}
                              </span>
                            )}
                          </button>
                        ...
                        {!isCollapsed ? (
                          booksViewMode === 'compact' ? (
                            <div
                              className="grid items-start gap-4 px-3 pt-3 pb-1"
                              style={{
                            ...
                            >
                              {group.books.map((book, bookIndex) => {
                                ...
                                const _bookRow =
                                  _bookProvider && _bookProviderId
                                    ? monitoredBookRowByKey.get(
                                        `${_bookProvider}:${_bookProviderId}`,
                                      )
                                    : undefined;
                                const ebookStatus = _bookRow
                                  ? getFormatStatus(_bookRow, 'ebook')
                                  : null;
                                const audiobookStatus = _bookRow
                                  ? getFormatStatus(_bookRow, 'audiobook')
                                  : null;
                                const isPrimaryGroup =
                                  group.key === '__standalone__' ||
                                  group.key === (book.series_name || '').trim();
                                let groupSeriesPos = book.series_position;
                                let groupSeriesCount = book.series_count;
                                if (!isPrimaryGroup && book.additional_series) {
                                  const match = book.additional_series.find(
                                    (a) => (a.name || '').trim() === group.key,
                                  );
                                  groupSeriesPos = match
                                ...
                                const seriesName = (
                                  book.series_name ||
                                  (group.key !== '__standalone__' ? group.title : '') ||
                                  ''
                                ).trim();
                                const seriesLabel =
                                  seriesName && groupSeriesPos != null
                                    ? `${seriesName} #${groupSeriesPos}`
                                    : seriesName;
                                ...
                                const showPopularity =
                                  booksCompactMinWidth >= 194 &&
                                  (popularity.rating !== null || popularity.readersCount !== null);
                                ...
                                const popularityLine = [
                                  ...
                                  popularity.readersCount !== null
                                    ? `${popularity.readersCount.toLocaleString()} readers`
                                    : null,
                                ]
                                  .filter(Boolean)
                                  .join(' • ');
                                ...
                                const isUpcoming = _bookRow
                                  ? isMonitoredBookUpcoming(_bookRow, _upcomingTodayMs)
                                  : false;
                                const authorCountdown =
                                  isUpcoming && _bookRow ? getUpcomingCountdown(_bookRow) : null;
                                return (
                                  <div
                                    key={book.id}
                                  ...
                                  >
                                    <MonitoredBookCompactTile
                                      ...
                                      seriesPosition={groupSeriesPos}
                                      seriesCount={groupSeriesCount}
                                      ebookStatus={ebookStatus}
                                      audiobookStatus={audiobookStatus}
                                      seriesLabel={seriesLabel}
                                      showSeriesName={showSeriesName}
                                      metaLine={isRemovedCard ? 'Removed from Hardcover' : metaLine}
                                      showMetaLine={isRemovedCard || Boolean(metaLine)}
                                      ...
                                      overflowMenu={
                                        monitoredEntityId ? renderBookOverflowMenu(book) : null
                                      }
                                      isDimmed={isDormant || isRemovedCard}
                                      isUpcoming={isUpcoming}
                                      countdownTag={authorCountdown}
                                      onEbookSearch={
                                        onGetReleases
                                          ? () => {
                                              void triggerReleaseSearch(
                            ...
                            <div className="divide-y divide-gray-200/60 dark:divide-gray-800/60">
                              {monitoredEntityId ? (
                                <div className="hidden grid-cols-[auto_auto_minmax(0,2fr)_minmax(190px,190px)_minmax(90px,90px)] items-center px-1.5 pt-1 pb-2 sm:grid sm:gap-x-2 sm:gap-y-1 sm:px-2">
                                  <div />
                                  <div />
                                  <div />
                              ...
                              {group.books.map((book, bookIndex) =>
                                (() => {
                                  ...
                                  const showSeriesInfo =
                                    Boolean(seriesLabel) && group.key !== '__standalone__';
                                  const isPrimaryGroup =
                                    group.key === '__standalone__' ||
                                    group.key === (book.series_name || '').trim();
                                  let groupSeriesPos = book.series_position;
                                  ...
                                  const _tRow =
                                    _tProvider && _tProviderId
                                      ? monitoredBookRowByKey.get(`${_tProvider}:${_tProviderId}`)
                                      : undefined;
                                  const isRemovedFromProvider =
                                    _tRow?.state === 'removed_from_provider';
                                  const tEbookStatus = _tRow
                                    ? getFormatStatus(_tRow, 'ebook')
                                    : null;
                                  const tAudiobookStatus = _tRow
                                    ? getFormatStatus(_tRow, 'audiobook')
                                    : null;
                                  return (
                                    <div
                                      key={book.id}
                                    ...
                                    >
                                      <MonitoredBookTableRow
                                        ...
                                        hasActiveSelection={
                                          monitoredEntityId ? hasActiveBookSelection : false
                                        }
                                        leadingControl={
                                          monitoredEntityId
                                            ? (() => {
                                                ...
                                                return (
                                                  <button
                                                    ...
                                                    title={
                                                      isSelected ? 'Unselect book' : 'Select book'
                                                    }
                                                  >
                                                    {isSelected ? (
                                                      <svg
                                        ...
                                        titleRow={
                                          <div className="flex min-w-0 items-center gap-1.5">
                                            ...
                                            {isRemovedFromProvider ? (
                                              <span className="shrink-0 rounded bg-red-500/20 px-1.5 py-0.5 text-[9px] font-semibold tracking-wide text-red-600 uppercase dark:text-red-400">
                                                Removed
                                              </span>
                                            ) : null}
                                            {hasSeriesPosition ? (
                                              <span
                                                className="hidden shrink-0 rounded bg-emerald-600 px-1 py-0 text-[9px] font-bold text-white sm:inline-flex sm:text-[10px]"
                                                style={{
                                            ...
                                            {monitoredEntityId ? (
                                              <span className="inline-flex flex-shrink-0 items-center gap-0.5 md:hidden">
                                                {tEbookStatus && (
                                                  <span title={`eBook: ${tEbookStatus}`}>
                                                    <svg
                                                      className={`h-3 w-3 ${tEbookStatus === 'available' ? 'text-emerald-500' : tEbookStatus === 'missing' ? 'text-red-400' : 'text-amber-400'}`}
                                                      fill="none"
                                                      stroke="currentColor"
                                                      viewBox="0 0 24 24"
                                                      strokeWidth={1.8}
                                                ...
                                                {tAudiobookStatus && (
                                                  <span title={`Audiobook: ${tAudiobookStatus}`}>
                                                    <svg
                                                      className={`h-3 w-3 ${tAudiobookStatus === 'available' ? 'text-emerald-500' : tAudiobookStatus === 'missing' ? 'text-red-400' : 'text-amber-400'}`}
                                                      fill="none"
                                                      stroke="currentColor"
                                                      viewBox="0 0 24 24"
                                                      strokeWidth={1.8}
                                        ...
                                        subtitleRow={
                                          <p className="truncate text-xs text-gray-600 sm:text-sm dark:text-gray-300">
                                              ...
                                              <div className="hidden items-center gap-2 text-xs text-gray-500 sm:flex dark:text-gray-400">
                                                {popularity.rating !== null ? (
                                                  <span className="inline-flex items-center gap-1 whitespace-nowrap">
                                                    ...
                                                    <span>
                                                      {popularity.rating.toFixed(1)}
                                                      {popularity.ratingsCount !== null
                                                        ? ` (${popularity.ratingsCount.toLocaleString()})`
                                                        : ''}
                                                    </span>
                                                  </span>
                                                ) : null}
                                                  ...
                                                  <span className="inline-flex items-center gap-1 whitespace-nowrap">
                                                    ...
                                                    >
                                                      <path
                                                        strokeLinecap="round"
                                                        strokeLinejoin="round"
                                                        d="M15 19.128a9.38 9.38 0 0 0-3-.479c-1.07 0-2.098.18-3 .512m6 0a7.5 7.5 0 1 0-6 0m6 0a9.372 9.372 0 0 1 3 .512M9 10.5a3 3 0 1 1 6 0 3 3 0 0 1-6 0Z"
                                                      />
                                                    </svg>
                                                    <span>
                                                      {popularity.readersCount.toLocaleString()}
                                                    </span>
                                                  </span>
                                                ) : null}
                                              </div>
      ...
      {releaseDateBook && monitoredEntityId && (
        <ReleaseDateSearchModal
          ...
          onMatched={(releaseDate) => {
            setMonitoredBookRows((prev) =>
              prev.map((r) =>
                r.provider === releaseDateBook.provider &&
                r.provider_book_id === releaseDateBook.provider_book_id
                  ? {
                      ...r,
      ...
      <BookDetailsModal
        ...
        onToggleMonitor={
          activeBookDetails && monitoredEntityId
            ? (type) => void toggleBookMonitor(activeBookDetails, type)
            : undefined
        }
        monitorEbook={
          activeBookDetails ? getBookMonitorState(activeBookDetails).monitorEbook : undefined
        }
        monitorAudiobook={
          activeBookDetails ? getBookMonitorState(activeBookDetails).monitorAudiobook : undefined
        }
        hidden={activeBookDetails ? isBookHidden(activeBookDetails) : false}
        onToggleHidden={
          activeBookDetails && monitoredEntityId
            ? () => void toggleBookHidden(activeBookDetails)
            : undefined
        }
          ...
          actions={
            [
                ...
                ? [
                    {
                      ...
                      disabled: bulkDownloadRunningByType.ebook,
                      ...
                      menuItems: [
                        ...
                        },
                        {
                          ...
                          onClick: () => void runBulkInteractiveSearchForSelection('ebook'),
                        },
                      ],
                    },
                    {
                      key: 'dl-audiobook',
                      icon: <AudiobookIcon className="h-4 w-4" />,
                      title: 'Audiobooks',
                      onClick: () => void runBulkDownloadForSelection('audiobook'),
                      disabled: bulkDownloadRunningByType.audiobook,
                      borderColor: 'teal' as const,
                      menuItems: [
                        {
                        ...
                        },
                        {
                          ...
                          onClick: () => void runBulkInteractiveSearchForSelection('audiobook'),
                        },
                      ],
                    },
                  ]
                : []),
              {
                ...
                title: 'Monitoring',
                onClick: () => void bulkToggleMonitorForSelection(),
                borderColor: 'teal' as const,
                menuItems: [
                  {
                    label: 'Monitor eBook',
                    onClick: () =>
                      void Promise.allSettled(
                        selectedBooks.map((b) => toggleBookMonitor(b, 'ebook', true)),
                      ),
                  },
                  {
                    ...
                    onClick: () =>
                      void Promise.allSettled(
                        selectedBooks.map((b) => toggleBookMonitor(b, 'audiobook', true)),
                      ),
                  },
                  {
                    ...
                    onClick: () =>
                      void Promise.allSettled(
                        selectedBooks.map((b) => toggleBookMonitor(b, 'ebook', false)),
                      ),
                  },
                  {
                    ...
                    onClick: () =>
                      void Promise.allSettled(
                        selectedBooks.map((b) => toggleBookMonitor(b, 'audiobook', false)),
                      ),
                  },
                ],
              },
              {
                ...
                title: 'Move selected to hidden',
                onClick: () => void bulkHideSelectedBooks(),
